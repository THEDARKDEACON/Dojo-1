...
// over serial. It handles motor control and encoder feedback.
...
void processCommand(String command) {
  ...
  switch (cmd) {
    case 'M': // Motor command: M<left_speed>,<right_speed> (in rad/s)
      {
        int commaIndex = params.indexOf(',');
        if (commaIndex > 0) {
          // Parse target speeds in rad/s
          float newLeftSpeed = params.substring(0, commaIndex).toFloat();
          float newRightSpeed = params.substring(commaIndex + 1).toFloat();
    ...
    case '?': // Print help
      Serial.println("D=== ROS 2 Motor Controller ===");
      Serial.println("DCommands:");
      Serial.println("D  M<left>,<right> - Set motor speeds in rad/s");
      Serial.println("D  S - Stop all motors");
      Serial.println("D  Z - Zero encoders");
      Serial.println("D  ? - This help message");
      Serial.print("DCurrent limits: ±");
      Serial.print(MAX_RAD_PER_SEC, 1);
      Serial.println(" rad/s");
      break;
      
    default:
      Serial.print("DUnknown command: ");
      Serial.println(command);
      Serial.println("DType '?' for help");
  }
}
...
void sendEncoderData() {
  ...
  // Send data in format: E<left_ticks>,<right_ticks>,<left_speed>,<right_speed>,<left_pos>,<right_pos>
  Serial.print("E");
  Serial.print(leftEncoderTicks);
  Serial.print(",");
  Serial.print(rightEncoderTicks);
  Serial.print(",");
  Serial.print(leftCurrentSpeed, 4);  // rad/s
  Serial.print(",");
  Serial.print(rightCurrentSpeed, 4); // rad/s
  Serial.print(",");
  Serial.print(leftPosition, 6);     // meters
  Serial.print(",");
  Serial.println(rightPosition, 6);  // meters
}
  ...
  while (Serial.available() > 0) {
    ...
    if (inChar == '\n' || inChar == '\r') {
      ...
      if (inputString.length() > 0) {
        stringComplete = true;
        return; // Exit to process the command
      }
    } 
    // Only add printable ASCII characters
      ...
      // Prevent buffer overflow (max 64 characters per command)
      if (inputString.length() >= 64) {
        inputString = "";
        Serial.println("DError: Command too long (max 64 chars)");
        return;
      }
    }
    // Ignore other characters (like backspace, etc.)
  }
}
